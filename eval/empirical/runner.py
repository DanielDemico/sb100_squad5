...
def main():
    ...
    print(
        f" -> Acurácia Global: {clf_res['accuracy'] * 100:.2f}% ({clf_res['correct_count']}/{clf_res['total_questions']})"
    )
    ...
    print(
        f" -> Média de Interações até Reclassificar: {dyn_res['average_interactions_to_reclassify']}"
    )
    ...
    print(
        f" -> Score Leigo: {pers_res['leigo_mean']}, Caipira: {pers_res['caipira_mean']}, Técnico: {pers_res['tecnico_mean']}"
    )
    ...
    print(
        f" -> Score Médio das Justificativas: {exp_res['mean_score']}/5.0 (Justificativas Corretas: {exp_res['correct']})"
    )
    ...
    print(
        f" -> Taxa de Logs Completos: {trc_res['completeness_rate'] * 100:.2f}% | Status de Reconstrução: {trc_res['reconstruction_status']}"
    )
    ...
    print(
        f" -> Trata classes inválidas com segurança: {rob_res['invalid_classes_safely_handled']}/{rob_res['invalid_classes_tested']}"
    )
    ...
    print(
        f" -> Latência Total Média: {perf_res['total']['mean_ms']} ms | P95: {perf_res['total']['p95_ms']} ms | P99: {perf_res['total']['p99_ms']} ms"
    )
