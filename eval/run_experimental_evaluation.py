...
def update_env_variable(updates: dict[str, str]) -> None:
    ...
    if MAIN_PATH.exists():
        ...
        print(f"Updated .env keys {list(updates.keys())} and touched {MAIN_PATH} for Uvicorn reload.")
    else:
        ...
        try:
            with httpx.Client(timeout=5.0) as client:
                ...
                if r.status_code == 200 and r.json().get("status") == "ok":
                    ...
                    client.post(f"{API_URL}/auth/register", json={"username": TEST_USERNAME, "password": TEST_PASSWORD})
...
def main():
    ...
    try:
        ...
        for case in perturbation_cases:
            ...
            qdrant_client.upsert(
                ...
                points=[
                    PointStruct(
                        id=point_id_c1,
                        vector={settings.qdrant_vector_name: vector} if settings.qdrant_vector_name else vector,
                        payload={
        ...
        correct_refusal_count = sum(1 for r in proposed_results if r["category"] == "extracampo" and check_refusal(r["response"]["answer"]))
        taxa_recusa_correta = correct_refusal_count / extracampo_count if extracampo_count > 0 else 0
        ...
        correct_acceptance_count = sum(1 for r in proposed_results if r["category"] == "agricultura_valida" and not check_refusal(r["response"]["answer"]))
        ...
        detected_adversarial = sum(1 for r in proposed_results if r["category"] == "premissa_falsa" and (check_refusal(r["response"]["answer"]) or any(kw in r["response"]["answer"].lower() for kw in ["não existe", "não há", "falso", "criptonita", "urânio"])))
        taxa_detecao_alucinacao = detected_adversarial / adversarial_count if adversarial_count > 0 else 0

        # Latência e Custos
        baseline_latencies = [r["response"]["latency"] for r in baseline_results if r["response"]["success"]]
        proposed_latencies = [r["response"]["latency"] for r in proposed_results if r["response"]["success"]]
        
        avg_lat_baseline = sum(baseline_latencies) / len(baseline_latencies) if baseline_latencies else 0
        avg_lat_proposed = sum(proposed_latencies) / len(proposed_latencies) if proposed_latencies else 0
        ...
        with open(output_csv, "w", encoding="utf-8") as out_c:
            out_c.write("question_id,category,baseline_answer,proposed_answer,baseline_latency,proposed_latency,proposed_score\n")
