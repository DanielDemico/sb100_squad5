...
def generate_latency_comparison(data: dict) -> None:
    ...
    fig, ax = plt.subplots(figsize=(6, 5))
    bars = ax.bar(['Baseline (Sem Validação)', 'Pipeline Proposto (Com Validação)'], 
                  [avg_b, avg_p], 
                  color=['#3b82f6', '#10b981'], 
                  width=0.5)
...
def generate_latency_distribution(data: dict) -> None:
    ...
    ax.boxplot([b_lats, p_lats], patch_artist=True,
...
def generate_perturbation_scores(data: dict) -> None:
    ...
    rects2 = ax.bar(x + width/2, c2_scores, width, label='C2 (Contexto Perturbado)', color='#ef4444')
